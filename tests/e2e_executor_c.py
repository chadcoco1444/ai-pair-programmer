...
Usage:
    python3 tests/e2e_executor_c.py [--executor-url http://localhost:4000]
...
SOLUTIONS_DIR = os.path.join(os.path.dirname(__file__), "solutions_c")
...
def submit_to_executor(code, test_cases, timeout=30000):
    payload = {
        "submissionId": f"e2e-c-{int(time.time()*1000)}",
        "language": "C",
        "code": code,
...
def main():
    ...
    solution_files = sorted(glob.glob(os.path.join(SOLUTIONS_DIR, "*.c")))
    ...
    for sol_file in solution_files:
        ...
        if status == "ACCEPTED":
            ...
            if result.get("compileError"):
                print(f"        compile: {result['compileError'][:300]}")
            if result.get("error"):
